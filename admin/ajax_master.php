...
if ( isset($_POST['action']) && $_POST['action'] == 'update_time_table'){
  ...
  $db->select_one("tbl_time_table","training_dt,session_no,period_type,break_time,class_start_time,class_end_time,faculty_id,paper_id,detail_topic_id,paper_covered,session_type,other_class,class_remark",$update_id);
  $res = $db->getResult();
 //print_r($res);
   if($res){
        foreach($res as $row){
         // $oldTable = $row;
            ...
            $db->select("tbl_modifytimetable","*",null,"time_table_id='".$update_id."' AND session_no = '".$row['session_no']."' AND training_dt = '".$row['training_dt']."' ",null,null);
              ...
              if($res1){
                ...
                $insert_sql = "INSERT INTO `tbl_modifytimetable` (`time_table_id`,`training_dt`, `session_no`,`period_type`, `break_time`, `class_start_time`, `class_end_time`, `faculty_id`, 
                                ...
                                VALUES ('".$update_id."','".$row['training_dt']."','".$row['session_no']."','".$row['period_type']."', '".$row['break_time']."','".$row['class_start_time']."', '".$row['class_end_time']."', '".$row['faculty_id']."',
      ...
      foreach($res as $row){
          ...
          //exit;
            $db->update_dir($sql);
                ...
                if($res3){
                    //echo "success#".$res3[1];
                    $db->update("tbl_modifytimetable",['status'=> 3],"time_table_id='".$tbl_id."' AND session_no = '".$session_no."' AND training_dt = '".$trng_dt."'");
  ...
  if($res){
      ...
      $db->select($table,"*",null,"status = 1  AND paper_id =".$paper_id,null,null);
          ...
          foreach($res2 as $row2){
           ...
           // echo $row2['id'];
            ?>
echo <option value=<?php echo  $row2['id'] ?>
    style="display: <?php if(in_array( $row2['id'] , $subject)){echo 'none';}  ?> "><?php echo  $row2['subject_name'] ?>
</option>
<?php
...
if( isset($_POST['action']) && $_POST['action'] == 'email_docs'){
  ...
  foreach($db->getResult() as $row){
     // print_r($row);
      ...
      ?>
latter : <a href="<?php echo "email_doc/".$row['latter']; ?>" target="_blank">latter <img
        src="../images/document_pdf.png" /></a><br>
Annexure 1 : <a href="<?php echo "email_doc/".$row['anx1']; ?>" target="_blank">Annexure1 <img
        src="../images/document_pdf.png" /></a><br>
...
if( isset($_POST['action']) && $_POST['action'] == 'edit_faculty'){
   ...
   if($_FILES['photo']['name'] == ''){
    $update_data = ["name" => $frm_data['name'],"desig" => $frm_data['desig'],"cader" => $frm_data['cader'],"qulftn" => $frm_data['qulftn'],
...
$sql = "INSERT INTO `tbl_program_directors` (`id`, `program_id`,`trng_type`, `course_director`) VALUES 
        (NULL, '$program_id', '$trng_type', '$faculty_id') ";
$db->insert_sql($sql);
...
if($last_insert_id > 0){
$db->update($table,["course_director"=>$last_insert_id],'id='.$program_id);
$res = $db->getResult();
if($res){
  $sql = "SELECT u.id,u.roll_id FROM `tbl_faculty_master` f JOIN `tbl_user` u  ON f.phone=u.username  WHERE f.id  = $faculty_id";
   $db->select_sql($sql);
   foreach($db->getResult() as $row1){
   
     $roll = $row1['roll_id'].','.$roll_id;

     $update_sql = "UPDATE `tbl_user` SET `roll_id` = '$roll' WHERE `tbl_user`.`id` = (SELECT u.id FROM `tbl_faculty_master` f JOIN `tbl_user` u  ON f.phone=u.username  WHERE f.id  = $faculty_id)";
     $db->update_dir($update_sql);
     $update_res = $db->getResult();
     if($update_res[0] == 1){
      echo  "success";
...
if( isset($_POST['action']) && $_POST['action'] == 'add_asst_course_dir'){
  ...
  $table = $_POST['table'];
  $trng_type = $_POST['trng_type'];
  $course_dirs_id = $_POST['course_dir'];

  $asst_course_dir = 0;
  //print_r($_POST);
  
   $db->select('tbl_program_directors','*',null,"id =".$course_dirs_id,null,null);
    foreach( $db->getResult() as $asst_dir){
      print_r($asst_dir);
      $asst_course_dir = $asst_dir['asst_course_director'];
    }
   // echo $asst_course_dir;
    if($asst_course_dir == 0){

      $db->update('tbl_program_directors',["asst_course_director"=>$faculty_id],'id='.$course_dirs_id);
      $update_asst_course_dir = $db->getResult();
      
      if($update_asst_course_dir){
      ...
      $db->update('tbl_program_directors',["asst_course_director"=>$faculty_id],'id='.$course_dirs_id);
      $update_asst_course_dir_res = $db->getResult();

      if($update_asst_course_dir_res){
      
        $sql = "SELECT u.id,u.roll_id FROM `tbl_faculty_master` f JOIN `tbl_user` u  ON f.phone=u.username  WHERE f.id  = $asst_course_dir";
        $db->select_sql($sql);
        foreach($db->getResult() as $row1){
          ...
          $update_sql = "UPDATE `tbl_user` SET `roll_id` = '$new_roll' WHERE `tbl_user`.`id` = (SELECT u.id FROM `tbl_faculty_master` f JOIN `tbl_user` u  ON f.phone=u.username  WHERE f.id  = $asst_course_dir)";
   ...
   // print_r($res2);
    ...
    // if($res2){
    //     $db->update($table,["asst_course_director"=>$asst_course_dir_id],'id='.$program_id);
    //     $res = $db->getResult();
...
if( isset($_POST['action']) && $_POST['action'] == 'update_course_dir'){
  ...
  $old_faculty_id = 0;
   ...
   exit;
    ...
    $db->select('tbl_program_directors',"course_director",null," id = '".$director_id."' ",null,null);
   
    foreach($db->getResult() as $faculty){
      $old_faculty_id = $faculty['course_director'];
    }
    
    //select roll id of old course director for update 
    
    $db->update('tbl_program_directors',["course_director"=>$faculty_id],'id='.$director_id);
    $update_res = $db->getResult();
